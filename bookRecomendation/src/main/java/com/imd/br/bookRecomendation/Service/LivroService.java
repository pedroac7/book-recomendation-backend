...
public class LivroService {
    ...
    public void salvarLivros(String jsonResponse) throws Exception {
        ...
        for (JsonNode doc : docs) {
            ...
            livro.setAutor(doc.path("author_name").isArray() ?
                    doc.path("author_name").get(0).asText() : "Desconhecido");
            livro.setGenero(doc.path("subject").isArray() ?
                    doc.path("subject").get(0).asText() : "Desconhecido");
